...
const newClients = clients.map(client => {
    const purchasesAmount = getTotalPurchasesAmount(getUserPurchases(purchases, client.id))
    const paymentsAmount = getTotalPaymentsAmount(getUserPayments(payments, client.id))
    return {
        ...client,
...
server.get('/clientsides/:clientId',(req,res)=>{
    ...
    const  clientsFilter = newClients.find(client=>client.id === parseInt(clientId))
    return res.json(clientsFilter)

})

server.get('/paymentswithdiff', (req, res) => {
    return res.json(getLastPaymentsAmount)
})
...
server.get('/lastpayments', (req, res) => {
 ...
 server.get('/payments/:clientId',(req,res)=>{
     const paymentsFilter = paymentsWithDiff.filter(payment=>payment.clientId === parseInt(req.params.clientId))
...
server.get('/purchases/:clientId',(req,res)=>{
    const purchasesFilter = purchases.filter(purchase=>purchase.clientId === parseInt(req.params.clientId))
